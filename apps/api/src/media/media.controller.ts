...
export class MediaController {
  ...
  @Public() @Get('public/*') async publicContent(@Req() request: Request, @Res() response: Response) { const rawPath = (request.params as unknown as Record<string, string>)[0] as string; const result = await this.media.readByKey(rawPath); response.setHeader('Content-Type', result.mimeType); response.setHeader('Cache-Control', 'public,max-age=31536000,immutable'); response.send(result.body); }
}
