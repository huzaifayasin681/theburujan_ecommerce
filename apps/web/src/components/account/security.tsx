...
export function AccountSecurity() {
  ...
  const emailMutation = useMutation({ mutationFn: (form: FormData) => api<{ message: string }>('/auth/update-email', { method: 'POST', body: JSON.stringify(Object.fromEntries(form)) }), onSuccess: (result) => { setMessage(result.message); setErrorMsg(''); }, onError: (err: Error) => setErrorMsg(err.message) });
  const setupTwoFactor = useMutation({ mutationFn: () => api<{ secret: string; otpauthUrl: string }>('/auth/2fa/setup', { method: 'POST' }), onSuccess: setTwoFactorSetup, onError: (err: Error) => setErrorMsg(err.message) });
  const confirmTwoFactor = useMutation({ mutationFn: (form: FormData) => api<{ recoveryCodes: string[] }>('/auth/2fa/confirm', { method: 'POST', body: JSON.stringify(Object.fromEntries(form)) }), onSuccess: (result) => { setRecoveryCodes(result.recoveryCodes); setTwoFactorSetup(null); setMessage('Two-factor authentication is enabled. Save the recovery codes now.'); }, onError: (err: Error) => setErrorMsg(err.message) });
  const disableTwoFactor = useMutation({ mutationFn: (form: FormData) => api('/auth/2fa/disable', { method: 'POST', body: JSON.stringify(Object.fromEntries(form)) }), onSuccess: () => { setMessage('Two-factor authentication is disabled.'); setRecoveryCodes([]); }, onError: (err: Error) => setErrorMsg(err.message) });
    ...
    <div className="grid gap-12 mt-8 max-w-4xl">
      ...
      <div className="card p-6">
        ...
        <form action={(form) => emailMutation.mutate(form)} className="grid max-w-md gap-4">
          ...
          <button className="button w-fit" disabled={emailMutation.isPending}>Send verification link</button>
        </form>
      </div>
      ...
      <div className="card p-6">
        ...
        {!twoFactorSetup && <button className="button" onClick={() => setupTwoFactor.mutate()} disabled={setupTwoFactor.isPending}>Set up authenticator</button>}
